...
export default function ContentCalendar() {
  ...
  const { selectedClientPageId } = useCmsClient();
  ...
  // Adding an entry always goes through Compose now (same form/preview as everywhere else),
  // pre-filled with whichever date (and client) was clicked.
  const goToCompose = (date: Date | string) => {
    const d = typeof date === "string" ? date : dateKey(date);
    const clientParam = selectedClientPageId ? `&client=${encodeURIComponent(selectedClientPageId)}` : "";
    router.push(`/cms/compose?date=${d}${clientParam}`);
  };

  // "+ New post" in the sidebar links here with ?new=1 to jump straight to Compose for today.
  useEffect(() => {
    if (searchParams.get("new") === "1") {
      goToCompose(new Date());
    }
